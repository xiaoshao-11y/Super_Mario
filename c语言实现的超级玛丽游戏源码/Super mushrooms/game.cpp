...
private:
	...
	COINT coint[50];
	ENEMY enemy[20];
	IMAGE img_mapsky,img_p,img_map,img_ani,img_mapbk,img_home,im;
...
void game::start()
{
	...
	if(life==1||life==2)
		return;
	loadimage(0, "./bj.jpg", XSIZE, YSIZE);
	life=3;
	score=0;
	settextstyle(40,0,"黑体");	
	setbkmode(TRANSPARENT);
	settextcolor(RGB(255,131,70));
	RECT r1 = {0, 0, XSIZE, YSIZE/3};
	drawtext("超级关东煮", &r1, DT_CENTER | DT_VCENTER | DT_SINGLELINE);//文字水平垂直居中并显示在一行
	settextstyle(20,0,"楷体");
	settextcolor(RGB(134, 255, 150));
	setlinecolor(RGB(255, 250, 134));
	...
	while(flag1==1)
	{
		...
		switch(m.uMsg)//匹配鼠标信息
		{
		case WM_LBUTTONDOWN://左键点击
			EndBatchDraw();//停止批量绘制
		    if(m.x>XSIZE/2-45&&m.x<XSIZE/2+45&&m.y>YSIZE/3&&m.y<YSIZE/3+30&&flag1==1&&flag2==0&&flag3==0)//开始游戏
		    {
				flag1=0;
				break;
			}
			else if(m.x>XSIZE/2-45&&m.x<XSIZE/2+45&&m.y>YSIZE/3+30&&m.y<YSIZE/3+60&&flag1==1&&flag3==0)//游戏介绍
			{
			    flag2=1;
				setbkcolor(RGB(106, 181, 255));
				cleardevice();
				rectangle(50,50,213,220);
				outtextxy(52,52,"游戏介绍：");
				outtextxy(52,82,"超级关东煮");
				outtextxy(52,132,"开发者：");
				outtextxy(52,162,"关东组");
			...
			else if(m.x>XSIZE/2-45&&m.x<XSIZE/2+45&&m.y>YSIZE/3+60&&m.y<YSIZE/3+90&&flag1==1&&flag2==0)//操作说明
			{
				flag3=1;
				setbkcolor(RGB(106, 181, 255));
				cleardevice();
			...
			else if(m.x>XSIZE/2-45&&m.x<XSIZE/2+45&&m.y>YSIZE/3+90&&m.y<YSIZE/3+120&&flag1==1&&flag2==0&&flag3==0)//退出游戏
			    exit(0);
...
void game::init()
{
	...
	loadimage(&img_mapsky,"res\\mapsky.bmp",XSIZE,YSIZE*4);//加载天空
	loadimage(&img_p,"res\\role.bmp");//加载人物
	loadimage(&img_map,"res\\map.bmp");//加载地图障碍
	loadimage(&img_ani,"res\\ani.bmp");//加载怪物
	loadimage(&img_mapbk,"res\\mapbk.bmp");//加载最后场景
	loadimage(&img_home,"res\\home.bmp",XSIZE,YSIZE*5);//加载游戏结束画面
	loadimage(&im, "res\\香菇.jpg",XSIZE, YSIZE);
	//加载音乐
...
int game::is_l_touch(int id)                      //主角的左边是否碰到墙或敌人，以及敌人是否碰到陆地的左边界
{
	int x,y;
	int i;
	if(id==1)                                    //id==1表示主角是否碰到id为1的地图，即游戏中黄色的地图
	{
    	x=-xmap+role.x;                          
    	y=role.y;
    	for(i=0;i<350;i++)       
...
int game::is_r_touch(int id)                  //主角的右边是否碰到墙或敌人，以及敌人是否碰到陆地的右边界
{
	int x,y;
	int i;
	if(id==1)
...
int game::is_t_touch()                         //主角的头是否碰到墙
{
	...
	for(int i=0;i<350;i++)
	{
		if(map[i].id!=0&&map[i].id<4)
		{
			...
			if((x-m[1].x)/role.w==0&&y>m[0].y&&y<m[1].y)             //判断人物上方是否接触墙壁
				return 1;
		}
	}
	return 0;
}
int game::is_b_touch(int id)                //主角是否踩到敌人。
{
	if(id==1)                           //是否碰到黄色地图
	{
	    int x,y;
...
int game::is_land(ENEMY e)//判断怪物是否在陆地板块上
{
	POINT r[2];//将怪物的坐标信息存储在POINT里
	r[0].x=e.x;
	r[0].y=e.y+role.h;
	...
	for(int i=0;i<350;i++)
	{
		if(map[i].id!=0&&map[i].id<4)//查找地图板块
		{
			POINT m[3];
			//地图板块左上角点的坐标
			m[0].x=map[i].x;
...
void game::fall()           
{
	...
	if(v0>=0)   //自由落体
	{
		if(isdie()==1)//判断人物是否死亡
			return;    
		if(is_b_touch(1)==1)//判断是否踩到敌人 1为踩到敌人
		{
			v0=0;
			role.y=role.y/role.h*role.h;//修正人物高度
			role.jump=0;
		}
	}
	else       //向上跳跃
	{
		...
		if(is_t_touch()==1)//判断是否碰到天花板
		{
			v0=0;
			h=0;
			role.y=role.y/role.h*role.h+role.h;//实现人物反弹
		}
	}
	v0=v0+G*t;
}
void game::shoot()
{
	...
	for(i=0;i<20;i++)
	{
		if(bullet[i].id==1)
		{
			...
			if((bullet[i].x<(-3*role.w))||(bullet[i].x>XSIZE))//子弹射出屏幕
			{
		        bullet[i].id=0;//停用子弹
				bullet[i].x=-1;
	            bullet[i].y=-1;
...
int game::eat(BULLET b)
{
	...
	for(i=0;i<350;i++)
	{
		if(map[i].id!=0&&map[i].id<4)//遍历地图块
		{
			POINT m[2];
